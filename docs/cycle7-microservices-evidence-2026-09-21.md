...
根目录单体仍保留，用于复现 M0-M6 的并发实验。`microservices` 是独立 Maven Reactor，五个模块均可单独构建和启动。
...
最终订单状态：`PAID / RESERVED`，外部交易号 `PAY-CYCLE7-970001`。
...
- Outbox 使用数据库状态抢占与一分钟陈旧任务回收，已验证单实例和重复消费语义；多实例长时间故障注入仍需单独压测。
...
- 加固后的真实联调验证：四个端口均为 `UP`；无内部令牌调用库存变更接口返回 401，Gateway 上不存在
