...
1. 把超时关单迁入 Order Service，确保关单与支付并发仍由状态条件更新裁决。
2. 把 Redis 快速下单、异步命令和对账按服务所有权迁移。
3. 为 Order 与 Inventory 划分独立 schema 和账号权限，再验证补偿、迁移和回滚。
4. 接入服务发现，并补多实例 Outbox、故障注入和微服务可观测性。
